...
def get_last_product():
    ...
    mycursor.execute("SELECT name, sleep(5) FROM product ORDER BY id DESC LIMIT 1;")
    ...
    mycursor.execute("select @@hostname;")
    db_hostname = mycursor.fetchone()
    ...
    return f'{product_count[0]} products x (last={last_product[0]}) - dbserver={db_hostname[0]} (api server:{api_hostname})'
...
def save_order(order_id, status, product):
    ...
    print("inserted {order_id}, {status}, {product} in db")
